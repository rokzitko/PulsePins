...
class pio
{
 protected:
   loc data;
 public:
   pio(mm &dev, std::uintptr_t base) : data(dev.get_loc(base)) {}
};
...
class pio_in: public pio {
 ...
 private:
  ...
  loc edge;
 public:
   pio_in(mm &dev, std::uintptr_t base) :
     pio(dev, base),
     interruptmask(dev.get_loc(base, interruptmask_offset)),
     edge(dev.get_loc(base, edgecapture_offset))
   {}
   uint32_t read() {
     return data.read();
   }
