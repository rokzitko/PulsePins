...
class pll {
 ...
 public:
   pll(const mm &dev, const std::uintptr_t base, std::string name = "pll"s) :
     mode(dev, base,   0b000000*4, name + "/mode"), // rw: 0 = waitrequest, 1 = polling mode
     status(dev, base, 0b000001*4, name + "/status"), // read-only, 0 = busy, 1 = ready
     start(dev, base,  0b000010*4, name + "/start"), // write-only, write 0 or 1 to start reconfiguration
     N(dev, base,      0b000011*4, name + "/N"), // bypass-enable & odd/even division bits are read-only
     M(dev, base,      0b000100*4, name + "/M"),
     C(dev, base,      0b000101*4, name + "/C"),
     C0(dev, base,     0b001010*4, name + "/C0"), // counters
     C1(dev, base,     0b001011*4, name + "/C1"),
     // dynamic_phase_shift
     K(dev, base,      0b000111*4, name + "/K"), // write-only
     BW(dev, base,     0b001000*4, name + "/BW"),
     CP(dev, base,     0b001001*4, name + "/CP")
   {}
