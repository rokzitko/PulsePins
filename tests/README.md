...
* run_all_tests_forever: repeatedly runs all tests for burn-in testing (a log file named `report` is generated in the current directory; per-test logs go to `/var/volatile`; pass `-no-report-files` to skip the accumulating `report.run_N` files)
