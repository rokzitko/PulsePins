...
initial begin
  ...
  #1
  din_valid <= 1;
  din <= { REPLAY, 32'h0003, 32'h04 }; // replay 3 times (do not pass)
  #1
  din_valid <= 0;
...
initial begin
  ...
  @(posedge clk iff dout_valid);
  assert(dout[63:0] == 'h000000ff00000aff) else $fatal;
  @(posedge clk iff dout_valid);
  assert(dout[63:0] == 'h000000ff00000bff) else $fatal;
end
...
initial begin
  ...
  fh = $fopen("SUCCESS", "w");
  $fclose(fh);
  $set_coverage_db_name("run_pre_1.ucdb");
  $finish;
end
