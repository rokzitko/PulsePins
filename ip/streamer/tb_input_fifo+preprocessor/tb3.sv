...
initial begin
  ...
  #1
  wrreq <= 1;
  data <= { REPLAY, 32'h0001, 32'h01 }; // replay 1 element 1 time (do not pass)
  #1
  wrreq <= 0;
  // at least 2 wait states because of registered memory access
  #2
  wrreq <= 1;
...
initial begin
  ...
  fh = $fopen("SUCCESS", "w");
  $fclose(fh);
  $set_coverage_db_name("run_input_pre_3.ucdb");
  $finish;
end

endmodule: tb_input_pre_3
