...
| ---- | --------------- | --------------------- |
| Ingress | [`st_interface.sv`]({{ source_file("ip/streamer/st_interface.sv") }}) accepts Avalon-ST sequence data in `clk`; [`input_fifo.sv`]({{ source_file("ip/streamer/input_fifo.sv") }}) buffers it before decode. | ARM-side writes are decoupled from output timing. `asi_ready` is a transport backpressure signal, not an output-cycle promise. |
| Run-length decoding | [`rl_decoder.sv`]({{ source_file("ip/streamer/rl_decoder.sv") }}) runs in `clk`. While active and not backpressured, it can write one decoded update per `clk`; loading a new run can insert a bubble. | Decode throughput is not the same thing as pin timing. Decoded updates still cross into `streamer_clk`. |
...
| Trigger reset | `trigger_reset` returns the trigger chain to idle and resets the match latch while preserving the active trigger stage. | Releasing reset re-arms the same trigger condition; use retriggering when a sequence should advance to the next trigger subsequence. |
...
| Gate close | Closing the gate stops new output FIFO reads. Registered/read-side state may remain visible, and `qout` may hold its last value. | Do not expect a zero-latency visible pause. Use `qout_valid` or `qout_strobe` activity to detect advancement. |
...
| `qout_strobe` | [`output_fifo.sv`]({{ source_file("ip/streamer/output_fifo.sv") }}) derives the strobe from `qout_valid` and the inverted read clock. | `qout_strobe` pulses are tied to valid output-domain data, not to every held `qout` value. |
...
| Retrigger | A retrigger element raises `retrig_requested`; the trigger chain discards the just-satisfied active stage, returns to idle, and waits for the next queued trigger condition. | Retrigger pauses advancement through trigger state, not by rewinding already decoded FIFO contents or re-arming the same trigger stage. |
...
| [`st_mux_if.sv`]({{ source_file("ip/st_mux/st_mux_if.sv") }}) | Single `clk` domain. | Selected Avalon-ST `data`, `valid`, and `ready` are combinational; selected channel, counters, and readback are registered. In the HPS integration, the channel tag lets the generated 32-to-96 adapter keep partial streamer elements separate by source. |
| [`rl_encoder.sv`]({{ source_file("ip/rl_encoder_if/rl_encoder.sv") }}) | Sample input clock to control-side `clk` through a dual-clock FIFO. | Samples while `valid` is high, emits a run on value change, count saturation, or validity drop, then crosses the FIFO. |
