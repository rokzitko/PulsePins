...
| `done` | [`output_fifo.sv`]({{ source_file("ip/streamer/output_fifo.sv") }}) asserts `done` when the terminating element is read without a prior buffer error. | Completion is observed in the output domain after the terminator reaches the FIFO read side. |
...
| [`ts_core.sv`]({{ source_file("ip/ts_core/ts_core.sv") }}) | Timebase `clk`. | Asynchronous inputs use three-stage synchronizers; rising edges emit one-cycle valid pulses only when downstream is ready. Non-ready events are dropped. |
| [`freq_meter.sv`]({{ source_file("ip/freq_meter/freq_meter.sv") }}) | Input clocks to `cnt_clk` to Avalon `avs_clk`. | Edge counters cross by Gray-code synchronization; results update once per gate interval and then cross back by update toggles. |
