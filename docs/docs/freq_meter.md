...
The `clear` control behaves like a pulse request, while `enable` keeps periodic measurements running. A clear restarts the gate/reference-domain baseline for the next measurement and publishes zeroed result registers through the same update-toggle path used by completed gates. After CDC propagation, result reads return zero until the next gate completes.
...
* `read()` - read the latest published raw result snapshot
