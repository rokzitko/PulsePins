...
* Timeline Composer: a dependency-free browser-side editor for simple multi-channel pulse timelines in raw clock cycles
...
The **Timeline Composer** is a browser-only helper layered on top of the existing sequence text path. It lets users define named output-bit channels and pulse intervals in raw clock cycles, validates that pulses on the same channel do not overlap, previews the result as an SVG timing diagram, and writes ordinary `d <cycles> <value>` records into the **Sequence** text area once the browser has a status snapshot. Unassigned output bits preserve the current tracked qout state, timeline-owned bits are driven high only during their pulses, and no `final` record is generated by the browser because the backend appends the current tracked qout final value.
