...
``c++/pll_rules.hh``, or as a frequency string such as ``66M``. Existing presets are preserved exactly. If a value is neither a preset nor raw parameters, the runtime uses the same Cyclone V PLL calculator as [`pllcalc`](pllcalc.md), prints a warning with the calculated parameters, and then programs the PLL.
