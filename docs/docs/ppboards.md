...
They enable custom interfaces to the DE10-Nano SoC FPGA. As an example, a KiCad design for
...
signal; optional built-in 50 Ω terminators (enabled by jumpers)
* SMA connector for one trigger signal; it is connected to a fast comparator with a tunable
threshold voltage; optional 50 Ω terminator; monitoring LED
...
is frequency tuning of an OCXO; with PPS from a GNSS receiver, PulsePins can serve as a simple
