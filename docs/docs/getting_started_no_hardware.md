...
Build and test the Python bindings on a development machine:

```bash
make -C python USE_PREGENERATED=1 build test-host
```

This local build uses the checked-in pregenerated HPS header from [`c++/artifacts/`]({{ source_file("c++/artifacts/") }}) and intentionally skips tests marked as hardware-only. It is useful for syntax/import/API validation, but it is not a supported
replacement for building the production Python modules natively on the DE10-Nano. True Python
...
* exact runtime behavior of deployment scripts on the target-board image
